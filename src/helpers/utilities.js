...
import { convertAmountToRawNumber, convertStringToHex } from "./bignumbers";
...
export function formatCurrentAcount(currentAccount) {
  if(currentAccount){
...
export function countDown(countDownDate, spanID, executeNext, show) {
    let x = setInterval(function () {
      const daysCount= 1000 * 60 * 60 * 24;
      const hoursCount= 1000 * 60 * 60;
      const minutesCount = 1000 * 60;
      let now = new Date().getTime();
      let distance = countDownDate - now;
      let days = Math.floor(distance / daysCount);
      if(days < 10 )days = "0" + days
      let hours = Math.floor((distance % (daysCount)) / hoursCount);
      if(hours < 10)hours = "0" + hours
      let minutes = Math.floor((distance % (hoursCount)) / minutesCount);
      if(minutes < 10)minutes = "0" + minutes
      let seconds = Math.floor((distance % (minutesCount)) / 1000);
      if(seconds < 10)seconds = "0" + seconds
      if (show) {
        if(document.getElementById(spanID)){
          document.getElementById(spanID).textContent = days + " D : " + hours + " H : " + minutes + " M : " + seconds + " S";
        }
      }
      if (distance <= 0) {
        executeNext(x);
